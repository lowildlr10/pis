...
function generateAbstract($pid, $conn, $chairman, $viceChairman,
	...
	foreach ($groupNumber as $grpNo) {
		...
		$pdf->Cell($pageHeight * 0.1116,'2',"  Revision 1","LR");
		$pdf->Ln();
		$pdf->SetFont('Arial','', 8);
		$pdf->Cell($pageHeight * 0.8334,'4',"",'');
		$pdf->Cell($pageHeight * 0.1116,'5','  02-28-18',"LR");
		$pdf->Ln();
		$pdf->Cell($pageHeight * 0.8334,'2',"",'');
		...
		$pdf->Cell($pageHeight * 0.948, '5', "Date: " . $abstractDate, "", "", "R");
		$pdf->Ln();

		// TABLE TITLE
		$pdf->SetFont('Arial','', 10 + ($increaseFontSize * 10));
		$pdf->Cell($pageHeight * 0.948, 21.75, 'ABSTRACT OF BIDS AND QUOTATION', "", "",'C');
		$pdf->Ln();
	...
	// 1ST ROW OF CELL/S
		...
		$titleTable = array("ITEM NO.", "QNTY", "UNIT", "PARTICULARS");
		$fontStyle = array("", "", "", "");
		...
		$columnWidths = array($columnTitleWidth[0], $columnTitleWidth[1], $columnTitleWidth[2], 
							  $columnTitleWidth[3]);
	...
	// DATA
		...
		// BIDS AND AWARDS COMMITTEE
			...
			if ($toggleAlternateMember == "Yes") {
				...
				$pdf->SetFont('Arial', '', 10 + ($increaseFontSize * 10));
				$pdf->Cell($totalWidthDisplay * 0.25, 5, "Approved by:", '', 0, 'L', 0);
				$pdf->Ln(); // LINE BREAK
				$pdf->SetFont('Arial', 'B', 9 + ($increaseFontSize * 9));
				$pdf->Cell(0, 8, " ", '', 0, 'L', 0);
				$pdf->Ln(); // LINE BREAK

				$fontStyle = array("B", "B", "B", "B", "B", "B");
				$fontSize = array(9, 9, 9, 9, 9, 9);
				$columWidth = $totalWidthDisplay / 6;
				$columnWidths = array($columWidth, $columWidth, $columWidth, $columWidth, $columWidth, $columWidth);
				$aligns = array('C', 'C', 'C', 'C', 'C', 'C');
				$items = array($chairman, $viceChairman, $member1, $member2, $alternateMember, $regionalDirector);
				...
				$fontStyle = array("", "", "", "", "", "");
				$items = array("Chairman", "Vice Chairman", "Member", "Member", "Alternate Member", "Regional Director");
				$pdf->CustomRow2($items, 5, $fontStyle, $fontSize, $toggleBorders = 0);
				unset($items);
				unset($fontStyle);
				unset($fontSize);
				...
				if ($toggleSecondMember == "Yes") {
					...
					$pdf->SetFont('Arial', '', 10 + ($increaseFontSize * 10));
					$pdf->Cell($totalWidthDisplay * 0.32, 5, "Approved by:", '', 0, 'L', 0);
					$pdf->Ln(); // LINE BREAK
					$pdf->SetFont('Arial', 'B', 9 + ($increaseFontSize * 9));
					$pdf->Cell(0, 8, " ", '', 0, 'L', 0);
					$pdf->Ln(); // LINE BREAK

					$fontStyle = array("B", "B", "B", "B", "B");
					...
					$aligns = array('C', 'C', 'C', 'C', 'C');
					$items = array($chairman, $viceChairman, $member1, $member2, $regionalDirector);
					...
					$fontStyle = array("", "", "", "", "");
					$items = array("Chairman", "Vice Chairman", "Member", "Member", "Regional Director");
					$pdf->CustomRow2($items, 5, $fontStyle, $fontSize, $toggleBorders = 0);
					//unset($items);
					$items = array();
					unset($fontStyle);
					...
					$items = array($chairman, $viceChairman, $member1, $regionalDirector);
					...
					$fontStyle = array("", "", "", "");
					$items = array("Chairman", "Vice Chairman", "Member", "Regional Director");
					$pdf->CustomRow2($items, 5, $fontStyle, $fontSize, $toggleBorders = 0);
					//unset($items);
					$items = array();
					unset($fontStyle);
			...
			$pdf->SetFont('Arial', 'B', 9 + ($increaseFontSize * 9));
			$pdf->Cell($pageHeight * 0.242, 5, $endUser, 'B', 0, 'C', 0);
			$pdf->SetFont('Arial', '', 9 + ($increaseFontSize * 9));
			$pdf->Ln(); // LINE BREAK
