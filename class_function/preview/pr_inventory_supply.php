...
function generateIOS($startDate, $endDate, $conn, $dir, $increaseFontSize = 0, $pageHeight = 297, $pageWidth = 210) {
	$items = array();
	$filename = " FM-FAS-PUR F20 [ $startDate - $endDate ].pdf";
	...
	while ($data = $qryPMF->fetch_object()) {
		...
		$items[] = array("", $data->description, $data->itemNo, $data->unitIssue, 
	...
	$pdf->Cell($pageHeight * 0.93, "5", "REPORT ON PHYSICAL COUNT OF INVENTORY", "", "", "C");
	$pdf->Ln();
	$pdf->Cell($pageHeight * 0.35, "5", " ", "", "", "C");
	$pdf->Cell($pageHeight * 0.04, "5", "as of ", "", "", "C");
	...
	$pdf->SetWidths(array($pageHeight * 0.06, $pageHeight * 0.2, $pageHeight * 0.07, $pageHeight * 0.09,
						  ...
						  $pageHeight * 0.09));
	$pdf->Row(array("ARTICLE", "DESCRIPTION", "STOCK", "UNIT OF",
					"UNIT VALUE", "BALANCE PER", "ON HAND PER", "SHORTAGE / AVERAGE",
					"REMARKS"));
