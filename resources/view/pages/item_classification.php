...
if (isset($_SESSION['log_admin'])) {
	...
	if (isset($_POST['delCheck'])) {
		while (list(,$val) = each($_POST['delCheck'])) {
			...
			$qryCheckSig = $conn->query("SELECT classID FROM tblinventory WHERE classID='".$cid."' LIMIT 1");

			if (!mysqli_num_rows($qryCheckSig)) {
				$conn->query("DELETE FROM tblitem_classifications WHERE classID='".$cid."'");
			} else {
				$cannot .= "<br />- ".$class;
			}
	...
	start_layout("DOST-CAR Procurement System","Items Classification");
?>
...
<div id="action">
	<div class="col-xs-12 col-md-12" style="padding: 0px; margin-bottom: 2px">
		...
		<div class="col-md-3" style="padding: 0px;">
			<div class="btn-group btn-group-justified">
				<a class="btn btn-primary operation" href="item_classification_op.php">Add Classification</a>
				<a class="btn btn-default operation" href="javascript: ifCheck();">Delete Classification</a>
			</div>
		</div>
	</div>
</div>
...
$perPage = $_SESSION['showPerPage'];
	...
	//display list
	$qry = "SELECT COUNT(classID) totalBut FROM tblitem_classifications";
	$qrySign = $conn->query("SELECT * FROM tblitem_classifications ORDER BY classification ASC LIMIT $limit");
	echo '<form name="frmSign" method="post">';
	echo '<table class="table" cellpadding="4" cellspacing="0" id="tblStyle" align="center" width="85%">
		  <tr><th>Items Classification</th></tr>
		  <tr><td>';
	...
	if(mysqli_num_rows($qrySign)){
		...
		while($data=$qrySign->fetch_object()){
			...
			<td><a href="item_class_op.php?edit='.$data->classID.'" title="Edit classification"><img class="img-button" src="../../assets/images/edit.png" alt="Edit" /></a></td>
