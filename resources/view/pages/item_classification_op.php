...
if (isset($_SESSION['log_admin'])) {
	...
	if(isset($_POST['txtSigna']) && !empty($_POST['txtSigna'])){
		...
		$checkExists = $conn->query("SELECT classID FROM tblitem_classifications WHERE classification='$classification'");
		if(mysqli_num_rows($checkExists)){
			$result = "Classification already exists.";
		}else{
		$table = new db_operation();
		$table -> initialize('tblitem_classifications');
			if(!isset($_POST['edit'])){
				$action_taken = "Added";
	...
	//update
		...
		$qryEdit = $conn->query("SELECT * FROM tblitem_classifications WHERE classID='".$_REQUEST['edit']."'");
	...
	//end update
start_layout("DOST-CAR Procurement System: Item Classification","".$action."","");
?>
...
<br />
  <div align="left" style="padding-left: 10px;"><a href="item_classification.php?limit=<?php echo $_SESSION['curPage'] ?>"  class="operation" >&lt;&lt;Back</a></div>
<form method="post" name="frmAdd" id="frmAdd" onSubmit="return check_input(this);">
  <table border="0" cellspacing="4" cellpadding="4" id="tblStyle" width="50%" align="center">
     <tr>
       <th colspan="2"> Item Classification</th>
     </tr>
	  <?php if(isset($result)){ ?>
      ...
      <tr>
        ...
        <input type="reset" name="btnReset" id="btnCancel" value="Cancel" onClick="javascript: location.replace('item_classification.php');">
        <?php
		if(isset($editThis))
